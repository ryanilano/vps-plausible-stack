...
> `sudo` + Docker access that you created beforehand. This guide uses `admin` as the
...
- [ ] Key login as `admin` works: from your Mac, `ssh admin@<vps-ip>` logs in
...
ssh admin@<vps-ip>          # should log in with your key
ssh root@<vps-ip>          # should be REFUSED (PermitRootLogin no)
```

If `admin` logs in cleanly, you're done — close the old session. If it fails,
